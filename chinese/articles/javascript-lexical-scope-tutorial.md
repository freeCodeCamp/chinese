...
1. 首先，编译器会检查`writeName()`的函数作用域是否定义了`fullName`变量。未找到，继续向上查找下一个`sayName()`作用域。
2. 第二步，编译器会在`sayName()`的函数作用域中查找，依然没找到，继续向上查找下一个`profile()`作用域。
3. 第三步，编译器会在`profile()`的函数作用域中查找，依然没找到，继续向上查找下一个全局作用域。
...
因此，下次你女（男）朋友打电话让你去他的私人作用域时，切记他们是在邀请你去他们的私人空间 😜!
...
**词法作用域（Lexical Scope）** 是定义表达式时所在的区间。
