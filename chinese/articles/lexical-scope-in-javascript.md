...
在理解词法作用域之前，必须首先知道 JavaScript 是如何执行程序的。在下一章节，我们会深入了解这两个步骤。
...
-   在第一行：JavaScript 引擎遇到一个叫做`doSomething`的函数。引擎在当前作用域查找，看是否有`doSomething`,如果有，则引用这个函数，没有则抛出一个 reference error。
